...
const UzbekistanHotel = () => {
  const [priceIncrease, setPriceIncrease] = useState(
    INCREASE_RATES_FOR_TOURISTS
  );

  const previousChargesRef = useRef({
    tashkentHotel__uzbekistan__standard: 0,
  });
  ...
  useEffect(() => {
    const calculateAndUpdateTotal = () => {
      ...
      const updateRoomCharge = (
        hotelKey: 'tashkentHotel__uzbekistan__standard',
      ...
      updateRoomCharge(
        'tashkentHotel__uzbekistan__standard',
        TASHKENT_HOTEL_RATES.uzbekistan.standard
      );
    ...
    formData.tashkentHotel__uzbekistan__standard,
    formData.tashkentHotel__uzbekistan__standard__room,
    ...
    <>
      <Grid xs={12} item>
        <div style={styles.input}>
          <Typography variant="h4">Uzbekistan Hotel </Typography>
          <FormGroup>
            <Controller
              name="tashkentHotel__uzbekistan__standard"
              control={control}
              defaultValue={false}
            ...
            {formData.tashkentHotel__uzbekistan__standard && (
              <Grid xs={12} item>
                <div style={styles.input}>
                  ...
                  <Controller
                    name="tashkentHotel__uzbekistan__standard__room"
                    control={control}
                    defaultValue={false}
                    render={({ field }) => (
                        ...
                        <RadioGroup {...field}>
                          <FormControlLabel
                            ...
                            label={`${ROOM_SIZES.SINGLE} - $${
                              TASHKENT_HOTEL_RATES.uzbekistan.standard.single +
                              priceIncrease
                            }`}
                          />
                          <FormControlLabel
                            ...
                            label={`${ROOM_SIZES.DOUBLE} - $${
                              TASHKENT_HOTEL_RATES.uzbekistan.standard.double +
                              priceIncrease
                            }`}
                          />
                        </RadioGroup>
...
export default UzbekistanHotel;
